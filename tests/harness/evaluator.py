...
class CodeEvaluator:
    ...
    def _check_imports(self, code: str, result: EvaluationResult) -> None:
        """Check import statements using AST for precision."""
        try:
            tree = ast.parse(code)
        except SyntaxError:
            return
    ...
    def _pattern_matches(self, code: str, pattern: CodePattern) -> bool:
        ...
        - For other code: require multiple characteristic lines to match
        """
        pattern_code = pattern.code.strip()
        
        # Remove comment lines from the pattern
        code_lines = [
            line.strip() for line in pattern_code.split('\n') 
        ...
        # For non-import patterns, require EXACT line matches for multiple lines
        # This prevents false positives from keyword-only matching
        return self._multi_line_pattern_matches(code, code_lines)
    
    def _import_pattern_matches(self, code: str, import_lines: list[str]) -> bool:
        """Check if import patterns match exactly in the code."""
        # Parse actual imports from the code using AST for accuracy
        try:
            tree = ast.parse(code)
        except SyntaxError:
            return False
    ...
    def _multi_line_pattern_matches(self, code: str, pattern_lines: list[str]) -> bool:
        """Check if multiple lines from a pattern exist in the code.
        
        Requires at least 2 significant lines to match, or if only 1 line,
        it must be a very specific match (not just common keywords).
        """
        ...
        # Require at least 2 matches for multi-line patterns
        # Or 1 match if pattern only has 1-2 significant lines
